...
def annotate_patch(ax, patch, label):
    ...
    if width < len(label) * 10:  # if our bar is too small, text goes to the left
...
terra = [('ASTER', 1999.9, today, 2, 1),
         ...
         (None, today, 2028, 4, 0.2)]

# sentinel-2
sentinel = [('Sentinel-2A MSI', 2015.5, today, 2, 1),
            (None, today, 2028, 2, 0.2),
            ('Sentinel-2B MSI', 2017.2, today, 2, 1),
            (None, today, 2028, 2, 0.2)]
...
# width = today - 1971
# free_box = Rectangle([1971, 0], width, 17, fc=None, fill=False, ec='r', lw=3, zorder=3)
